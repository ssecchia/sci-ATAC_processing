	...
	if includezeroes:
		for rec in bedtuple:
			...
			for read in reads:
				...
				if read_start >= int(rec[1]) and read_start <= int(rec[2]):
...
#### Modifications by SS
# With the help of James and Adam, we added a if statement so that the read is only counted if its 5' prime position is within the bed region. The standard script
# counts overlaps for the whole read, so a read could be counted twice or more if it overlaps with two or more bed regions.
