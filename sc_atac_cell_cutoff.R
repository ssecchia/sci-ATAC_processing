...
report2 = read.table(paste0(currexpt,".report.txt"),header=F,row.names = 1, stringsAsFactors = T)
colnames(report2) <- c("Tag","Total")
...
###Modifications log
# Specified input table to have no header and col1 (barcodes) as row names
# Assigned colnames to input table